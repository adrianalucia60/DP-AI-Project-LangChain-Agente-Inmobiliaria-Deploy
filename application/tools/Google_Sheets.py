...
GOOGLE_SHEETS_HEADER_ROW = int(os.getenv("GOOGLE_SHEETS_HEADER_ROW", "2"))
...
def _get_worksheet():
    ...
    logger.info(
        "Conectando a Google Sheets: worksheet=%s header_row=%s credentials_source=%s",
    ...
    if not credentials_info and not os.path.isfile(creds_path):
        raise FileNotFoundError(
            "No se encontró el archivo JSON de credenciales de Google Sheets."
        )
